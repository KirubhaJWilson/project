  ...
  return (
    <Router>
      <Box sx={{ flexGrow: 1 }}>
        ...
        <Container sx={{ mt: 4 }}>
          <Routes>
            <Route path="/login" element={token ? <Navigate to="/dashboard" /> : <Login setToken={handleLoginSuccess} />} />
            <Route path="/dashboard" element={
              token ? (
                role === 'clinician' ? <ClinicianDashboard key="clinician" /> : <PatientDashboard key="patient" />
              ) : (
                <Navigate to="/login" />
              )
            } />
